...
def calculate_metrics(labels, scores, cutoff=0.5, po_label=1):
    ...
    # my_metrics['AUC']=metrics.roc_auc_score(labels, scores)
    return my_metrics
...
def calculate_metrics_cv(cv_res, label_column=0, score_column=2, cutoff=0.5, po_label=1):
    ...
    metrics= np.concatenate(tuple((f for f in cv_res)), axis=0)
    # print(metrics,len(metrics[:,0]))
    metrics_list.append(calculate_metrics(metrics[:, label_column], metrics[:, score_column], cutoff=cutoff, po_label=po_label))
    return metrics_list
