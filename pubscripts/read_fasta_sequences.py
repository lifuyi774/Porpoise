...
def read_nucleotide_sequences(file):
    ...
    for fasta in records:
        array = fasta.split('\n')
        header, sequence = array[0].split('\t'), re.sub('[^ACGTU-]', '-', ''.join(array[1:]).upper())
        # header_array = header.split('|')
        # name = header_array[0]
        name = header[0]+'_'+header[1]
        # label = header_array[1] if len(header_array) >= 2 else '0'
        # label = '0' if re.search('P',header)== None else '1'
        label ='0'
...
def read_nucleotide_sequences2(file):
    if os.path.exists(file) == False:
        print('Error: file %s does not exist.' % file)
        sys.exit(1)
    with open(file) as f:
    ...
    for fasta in records:
        array = fasta.split('\n')
        header, sequence = array[0].split()[0], re.sub('[^ACGTU-]', '-', ''.join(array[1:]).upper())   #  re.sub()'-' 替换[`ACGTU-'] .join 用‘’连接array[1:] upper() 小写转换为大写
        # header_array = header.split('|')
        # name = header_array[0]
        name = header
        # label = header_array[1] if len(header_array) >= 2 else '0'
        ...
        fasta_sequences.append([name, sequence, label, label_train])
    return fasta_sequences
def read_nucleotide_sequences1(file):
    if os.path.exists(file) == False:
        print('Error: file %s does not exist.' % file)
        sys.exit(1)
    with open(file) as f:
