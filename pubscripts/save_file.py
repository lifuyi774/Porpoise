    ...
    with open(out, 'w') as f:
        ...
        for i in range(len(data)):
            ...
            for j in range(len(data[i])):
                ...
                f.write(str(data[i][j][0])+'\t'+str(data[i][j][2])+'\n')
    return None
def save_CV_result_binary1(data, out, info=None):
    with open(out, 'w') as f:
        if info:
            f.write('%s\n' % info)
        for i in range(len(data)):
            # f.write('# result for fold %d\n' %(i + 1))
            for j in range(len(data[i])):
                # f.write('%d\t%s\t%s\t%s\n' % (data[i][j][0], data[i][j][1], data[i][j][2], data[i][j][4]))
                f.write(data[i][j][0]+'\t'+data[i][j][1]+'\t'+data[i][j][3]+'\n')
